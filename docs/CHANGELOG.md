...
### Added
- AI / Agent 产品经理岗位雷达 Skill 增加限页来源保护规则：前 10 页等部分抓取结果只用于确认新增/仍在线，不再将未捕获到的旧岗位误判为下架。
- AI / Agent 产品经理岗位雷达 Skill 增加高失败率来源恢复手册，记录百度、蚂蚁集团、阿里系、快手、字节跳动和小米的成功抓取与重试经验。
- README 当前已支持公司表格中的公司名增加对应 CSV 链接，便于从项目页直接查看各来源岗位数据。
- 将 README 与静态看板中的岗位变动展示口径从“近 7 日岗位变更”调整为“近 3 日新增”，仅展示新增岗位并排除下架记录。
...
- AI / Agent 产品经理岗位雷达顶部统计改为横向指标条，支持公司数量增加后的横向滚动展示，避免竖向堆叠。
- 新增本机 Codex Skill `ai-agent-pm-job-radar`，用于让 Agent 按源站抓取手册自主刷新岗位数据并执行基础验证。
- AI / Agent 产品经理岗位雷达对官网未披露发布时间的岗位改用首次抓取录入时间，并将岗位变动模块从最近 3 个岗位日期扩展为最近 7 个岗位日期。
